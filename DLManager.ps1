...
<# FEATURES:
- Improved UI layout with status bar
- Fallback to text input if GUI fails
...
"@
    ...
    $labelUser.Text = "Enter UPNs (comma-separated):"
    ...
    $executeOperation = {
        param($action)
        $users = $textboxUsers.Text.Trim() -split ',' | ForEach-Object { $_.Trim() }
